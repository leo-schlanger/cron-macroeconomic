...
def calculate_priority(title: str, description: str, positive_keywords: list,
                       negative_keywords: list) -> tuple[float, list]:
    """
    Calcula a prioridade de uma notícia baseado nas keywords.
    Retorna (score, lista de keywords encontradas)
    """
    ...
    for kw in negative_keywords:
        if kw in text:
            return -1.0, []  # Score negativo = ignorar
    ...
    for kw in positive_keywords:
        if kw in text:
            matched.append(kw)
            # Peso maior para keywords no título
...
if __name__ == "__main__":
    ...
    console.print("\n[bold blue]Iniciando fetch de todas as fontes...[/bold blue]\n")

    results = fetch_all_sources()
    summary = get_fetch_summary(results)

    # Tabela de resultados
    table = Table(title="Resultados do Fetch")
    table.add_column("Fonte", style="cyan")
    table.add_column("Status", style="green")
    table.add_column("Total", justify="right")
    table.add_column("Novas", justify="right", style="yellow")
    table.add_column("Ignoradas", justify="right", style="dim")

    for r in results:
        status = "[green]OK[/green]" if r["success"] else f"[red]ERRO[/red]"
        table.add_row(
            r["source_name"][:40],
            status,
            str(r["news_count"]),
    ...
    # Resumo
    console.print(f"\n[bold]Resumo:[/bold]")
    console.print(f"  Fontes processadas: {summary['total_sources']}")
    console.print(f"  [green]Sucesso: {summary['successful']}[/green]")
    console.print(f"  [red]Falhas: {summary['failed']}[/red]")
    console.print(f"  Notícias encontradas: {summary['total_news']}")
    console.print(f"  [yellow]Novas notícias salvas: {summary['new_news']}[/yellow]")
    console.print(f"  Ignoradas (filtro): {summary['skipped']}")

    if summary["errors"]:
        console.print(f"\n[bold red]Erros:[/bold red]")
        for err in summary["errors"][:10]:  # Mostrar até 10 erros
            console.print(f"  {err['source']}: {err['error'][:60]}")
